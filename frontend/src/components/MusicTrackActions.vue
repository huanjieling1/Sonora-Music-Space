...
const stateKey = computed(() => props.track?.provider && props.track?.id
  ? `${props.track.provider}:${props.track.id}`
  : props.track?.id || '')
const liked = computed(() => Boolean(music.trackStates[stateKey.value]?.liked))
const saved = computed(() => Boolean(music.trackStates[stateKey.value]?.saved))

onMounted(loadState)
onBeforeUnmount(clearMessage)
watch(() => [props.track?.id, searchId.value], () => {
  clearMessage()
  loadState()
})
...
function showMessage(text, duration = 800) {
  ...
  messageTimer = window.setTimeout(clearMessage, duration)
}
...
async function loadState() {
  ...
  try {
    ...
    music.setTrackState(props.track, {
...
async function toggleLike() {
  ...
  try {
    ...
    music.setTrackState(props.track, { liked: !liked.value })
    showMessage(liked.value ? '已加入我喜欢的音乐' : '已取消喜欢')
  } catch (error) {
...
async function saveToPlaylist() {
  ...
  loading.value = true
  try {
    const result = await addTrack(selectedPlaylistId.value)
    const selected = editablePlaylists.value.find(item => item.id === selectedPlaylistId.value)
    music.recordPlaylistTrackAdded(props.track, result.data || selected)
    await recordSaveEvent()
    showMessage('已收藏到所选歌单')
...
async function createAndSave() {
  ...
  try {
    ...
    const result = await addTrack(created.data.id)
    music.recordPlaylistTrackAdded(props.track, result.data || created.data)
    await recordSaveEvent()
    newPlaylistName.value = ''
    showMessage(`已创建“${name}”并收藏歌曲`)
    dialogOpen.value = false
  } catch (error) {
    showMessage(error.message, 2500)
  } finally {
...
async function recordSaveEvent() {
  try {
    await sendEvent('SAVE')
  } catch {
    // 歌曲已经写入歌单；画像事件失败不应撤销用户刚完成的收藏。
  }
}
</script>
...
<template>
  <span class="track-actions" :class="{ compact }" @click.stop>
    <button class="like-button" :class="{ active: liked }" :disabled="!available || loading" :title="liked ? '取消喜欢' : '喜欢并加入我喜欢的音乐'" @click="toggleLike">
      <Heart :size="compact ? 15 : 17" :fill="liked ? 'currentColor' : 'none'" />
    </button>
    <button class="collection-button" :class="{ saved }" :disabled="!available || loading" :title="saved ? '已收藏，可继续添加到其他歌单' : '收藏到歌单'" @click="openCollection">
...
<style scoped>
.track-actions{position:relative;display:inline-flex;align-items:center;gap:5px}.track-actions>button{display:grid;width:34px;height:34px;place-items:center;border:0;border-radius:50%;background:transparent;color:#747987}.track-actions>button:hover{background:rgba(255,255,255,.08);color:#b8ff54}.track-actions>button.like-button.active,.track-actions>button.like-button.active:hover{background:rgba(255,77,103,.1);color:#ff4d67}.track-actions>button.collection-button.saved,.track-actions>button.collection-button.saved:hover{background:rgba(201,154,24,.14);color:#c99a18}.track-actions>button:disabled{opacity:.35}.track-actions.compact>button{width:29px;height:29px}.action-message{position:absolute;z-index:4;right:0;bottom:calc(100% + 8px);width:max-content;max-width:220px;border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:6px 8px;background:#20202a;color:#cfd1d8;font-size:9px;line-height:1.35;pointer-events:none;box-shadow:0 8px 24px rgba(0,0,0,.35)}.action-fade-enter-active,.action-fade-leave-active{transition:opacity .16s ease,transform .16s ease}.action-fade-enter-from,.action-fade-leave-to{opacity:0;transform:translateY(4px)}
